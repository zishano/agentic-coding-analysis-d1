...
def build_streaming_pair_map(conn) -> Dict[str, str]:
    """Build map of non-streaming request_id -> paired streaming request_id

    Returns a dict mapping non-streaming request IDs to their corresponding
    streaming request IDs based on:
    1. Streaming request occurs 1-30 seconds before non-streaming
    2. Same model
    3. Identical messages input (MD5 hash match)
    """
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, timestamp, body
        FROM requests
        ORDER BY timestamp
    """)
    ...
    # Build streaming pair map
    pair_map = {}

    for i, (req_id, ts, body_str) in enumerate(all_requests):
        try:
            body = json.loads(body_str)

            # Only process non-streaming requests
            ...
            # Look backwards for matching streaming request
            best_match = None
            best_time_diff = None

            for j in range(i - 1, max(-1, i - 50), -1):  # Check up to 50 previous requests
                stream_id, stream_ts, stream_body_str = all_requests[j]

                try:
                    stream_body = json.loads(stream_body_str)

                    # Must be streaming
                    if stream_body.get('stream') is not True:
                        continue

                    # Check time window (1-60 seconds before)
                    stream_time = datetime.fromisoformat(stream_ts.replace('Z', '+00:00'))
                    time_diff = (req_time - stream_time).total_seconds()

                    if time_diff < 1 or time_diff > 60:
                        continue

                    # Must be same model
                    if stream_body.get('model') != model:
                        continue

                    # Must have identical messages input
                    stream_messages = stream_body.get('messages', [])
                    stream_hash = hashlib.md5(
                        json.dumps(stream_messages, sort_keys=True).encode()
                    ).hexdigest()

                    if stream_hash != messages_hash:
                        continue

                    # Found a match! Use the closest one (smallest time diff)
                    if best_match is None or time_diff < best_time_diff:
                        best_match = stream_id
...
def process_conversation(conn, conversation_id: str, jsonl_path: Path, msg_id_map: Dict[str, str], streaming_pair_map: Dict[str, str], args, shared_hash_state: dict = None) -> Optional[Dict]:
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        for req_id, ts_str, body_str, resp_str in segment:
            ...
            # Flag to track if we successfully used a streaming pair
            used_streaming_pair = False

            # Check if this request has a paired streaming request
            if req_id in streaming_pair_map:
                stream_req_id = streaming_pair_map[req_id]

                # Fetch the paired streaming request
                stream_req = cursor.execute("""
                    SELECT timestamp, body, response
                    FROM requests
                    WHERE id = ?
                """, (stream_req_id,)).fetchone()

                if stream_req:
                    stream_ts_str, stream_body_str, stream_resp_str = stream_req

                    # Validate that the streaming response is complete
                    # Only use streaming if it has valid response data
                    if stream_resp_str:
                        try:
                            stream_resp = json.loads(stream_resp_str)
                            # Check if response has responseTime and body (indicates complete response)
                            if stream_resp.get('responseTime') is not None and stream_resp.get('body'):
                                stream_ts = datetime.fromisoformat(stream_ts_str.replace('Z', '+00:00'))
                                # Use the paired streaming request (it has complete data)
                                builder.process_request(stream_ts, stream_body_str, stream_resp_str, 'streaming')
                                used_streaming_pair = True
                        except:
                            pass  # Failed to parse, will fallback below

            # If we didn't use a streaming pair, use the original request
            if not used_streaming_pair:
...
def process_subagent(conn, agent_id: str, subagent_index: Dict[str, Dict],
                     msg_id_map: Dict[str, str], streaming_pair_map: Dict[str, str], args, shared_hash_state: dict = None) -> Optional[Dict]:
    """
            ...
            if stream_req:
                ...
                if stream_resp_str:
                    try:
                        ...
                        if stream_resp.get('responseTime') is not None and stream_resp.get('body'):
                            stream_ts = datetime.fromisoformat(stream_ts_str.replace('Z', '+00:00'))
                            # Use the paired streaming request
                            builder.process_request(stream_ts, stream_body_str, stream_resp_str, 'streaming')
                            used_streaming_pair = True
                    except:
                        pass  # Failed to parse, will fallback below
...
def process_conversation_with_subagents(conn, conversation_id: str, jsonl_path: Path,
                                        msg_id_map: Dict[str, str], streaming_pair_map: Dict[str, str], args,
    ...
    traces = process_conversation(conn, conversation_id, jsonl_path, msg_id_map, streaming_pair_map, args, shared_hash_state=shared_hash_state)
    ...
    for trace in traces:
        ...
        for spawn_ts, link in links_with_ts:
            ...
            subagent_trace = process_subagent(conn, agent_id, subagent_index, msg_id_map, streaming_pair_map, args, shared_hash_state=shared_hash_state)
...
def main():
    ...
    print(f"Building streaming/non-streaming pair map...")
    streaming_pair_map = build_streaming_pair_map(conn)
    print(f"  Found {len(streaming_pair_map):,} non-streaming requests with paired streaming requests")
    print()
    ...
    if args.conversation_id:
        ...
        if args.include_subagents:
            traces = process_conversation_with_subagents(
                conn, args.conversation_id, jsonl_path, msg_id_map, streaming_pair_map, args,
        ...
        for jsonl_path in sorted(jsonl_files):
            ...
            if args.include_subagents:
                traces = process_conversation_with_subagents(
                    conn, conv_id, jsonl_path, msg_id_map, streaming_pair_map, args,
                    subagent_index, parent_links, shared_hash_state=conv_hash_state
                )
            else:
                traces = process_conversation(conn, conv_id, jsonl_path, msg_id_map, streaming_pair_map, args, shared_hash_state=conv_hash_state)
